...
namespace audio_pipe {
    ...
    void WasapiInputStream::push_audio_bytes(
        const uint8_t * data,
        size_t size
    ) {
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        const size_t target_bytes =
            static_cast<size_t>(frame_size_) * block_align_;
    ...
    void WasapiInputStream::capture_loop() {
        ...
        while (running_.load()) {
            ...
            while (packet_frames > 0) {
                ...
                const size_t bytes =
                    static_cast<size_t>(num_frames) * block_align_;
